// ==== Time Palette — Sidebar Widget (with Phases + Team Summary) ====
(function (ns) {
  let widgetEl = null;
  let injected = false;
  ...
  ns.sidebarWidget = {
    ...
    update: async function (parsedEvents, viewRange) {
      ...
      renderWidget(projects, projectHours, projectEventCounts, phaseHours, phaseNames, totalHours, viewRange, teamData);
    },
  };
  ...
  function renderWidget(projects, projectHours, projectEventCounts, phaseHours, phaseNames, totalHours, viewRange, teamData) {
    if (!widgetEl) return;

    const isMonthView = viewRange && viewRange.viewType === 'month';
    const isWeekView = viewRange && viewRange.viewType === 'week';

    const activeProjects = projects.filter((p) => (projectEventCounts[p.id] || 0) > 0);
    ...
    if (activeProjects.length > 0) {
      let rowsHtml = '';
      activeProjects.sort((a, b) => (projectHours[b.id] || 0) - (projectHours[a.id] || 0));

      activeProjects.forEach((project) => {
        const hours = projectHours[project.id] || 0;
        const count = projectEventCounts[project.id] || 0;
        const display = isMonthView && hours === 0
          ? count + ' event' + (count !== 1 ? 's' : '')
          : ns.formatHours(hours);

        const barPercent = totalHours > 0 ? Math.max(4, (hours / totalHours) * 100) : 0;
        ...
        rowsHtml += `
          <div class="tp-project-row">
            ...
            <div class="tp-bar-fill" style="width:${barPercent}%; background:${safeCol}"></div>
          </div>
        `;
      ...
      html += `
        ...
        <div class="tp-total-row">
          ...
          <span class="tp-total-hours">${ns.formatHours(totalHours)}</span>
        </div>
      `;
    }
